...
def _require_avatar(user) -> str | None:
    ...
    if not user.current_avatar_id:
        return "You haven't selected an avatar yet. Use /start to choose one."
    return None
...
async def cmd_history(message: Message, session: AsyncSession) -> None:
    ...
    if not history:
        await message.answer("No messages in this dialog yet. Start chatting!")
        return

    lines = []
    for msg in history:
        prefix = "You" if msg["role"] == "user" else user.current_avatar.name
...
async def cmd_facts(message: Message, session: AsyncSession) -> None:
    ...
    if not facts:
        await message.answer(
            "No facts stored yet. Keep chatting and I'll start remembering things about you!"
        )
        return

    facts_text = "\n".join(f"• {f}" for f in facts)
    await message.answer(
        f"**Facts I remember about you** (with {user.current_avatar.name}):\n\n{facts_text}",
        parse_mode="Markdown",
    )
...
async def cmd_reset(message: Message, session: AsyncSession) -> None:
    ...
    await message.answer(
        f"Dialog reset! Cleared {count} messages.\n"
        "Long-term facts are preserved — I still remember you."
    )
